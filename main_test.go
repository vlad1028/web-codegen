...
func TestMyApi(t *testing.T) {
	...
	cases := []Case{
		...
		Case{ // получили ошибку общего назначения - код сам подставил 500
			Path:   ApiUserProfile,
		...
		Case{ // получили специализированную ошибку - код поставил статус 404 оттуда
			Path:   ApiUserProfile,
		...
		// ------
		Case{ // это должен ответить ServeHTTP - если ему пришло что-то неизвестное (например когда он обрабатывает /user/)
			Path:   "/user/unknown",
			Query:  "login=not_exist_user",
			Status: http.StatusNotFound,
			Result: CR{
